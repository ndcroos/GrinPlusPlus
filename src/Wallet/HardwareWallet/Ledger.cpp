...
*/
	...
	)
	{
		...
		auto [err, buffer] = transport_->exchange(APDU::CLA, APDU::INS_SIGN_SENDER, confirm, 0x00, payload);
		if (err != Error::SUCCESS)
			return {err, {}};
		return {err, std::vector<uint8_t>(buffer.begin() + 1, buffer.end())};
	}
	...
	)
	{
		...
		auto [err, buffer] = transport_->exchange(APDU::CLA, APDU::INS_SIGN_RECEIVER, confirm, 0x00, payload);
		if (err != Error::SUCCESS)
			return {err, {}};
		return {err, std::vector<uint8_t>(buffer.begin() + 1, buffer.end())};
	}
	...
	)
	{
		...
		auto [err, buffer] = transport_->exchange(APDU::CLA, APDU::INS_SIGN_FINALIZE, confirm, 0x00, payload);
		if (err != Error::SUCCESS)
			return {err, {}};
		return {err, std::vector<uint8_t>(buffer.begin() + 1, buffer.end())};
	}
